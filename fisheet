...
--// HTTP helper (supports exploit request() if available)
local function http_post_json(url, json)
    -- prefer exploit HTTP if present (works ke Discord)
    local ok, res
    ...
    -- fallback Roblox HttpService (biasanya tidak bisa ke Discord)
...
local function createGUI()
    local parentUi = (gethui and gethui()) or game:FindFirstChildOfClass("CoreGui") or PlayerGui
...
local function sendWebhook(data, retryCount)
    ...
    local payload = {
        ...
        embeds = {{
            ...
            footer = { text = "FishHook Notifier • " .. os.date("%X") },
        }}
    }

    local json = HttpService:JSONEncode(payload)
    local ok, resp = http_post_json(Config.WebhookURL, json)
    if ok then
...
local function parseFishCatch(message)
    -- [Server]: playername obtained a FishName (weight) with a 1 in XK chance!
...
local function monitorChat()
    ...
    if success and chatService and chatService.Chatted then
        -- Method 1: Chat service event
        chatService.Chatted:Connect(function(_, message)
        ...
        -- Method 2: Observe classic chat GUI
        local function hookChat()
            ...
            if not chatChannelParentFrame then return end
            ...
            scrollingFrame.ChildAdded:Connect(function(child)
                if not Config.Enabled then return end
                task.wait(0.1) -- wait message fill
                local label = child:FindFirstChild("TextLabel")
                if label and label.Text then
                    local fishCatch = parseFishCatch(label.Text)
                    if fishCatch then
                        processFishCatch(fishCatch)
                    end
                end
...
function processFishCatch(fishCatch)
    ...
    local sellPriceStr = tostring(fishData.SellPrice):reverse():gsub("(%d%d%d)", "%1,"):reverse():gsub("^,", "")
    ...
    local webhookData = {
        ...
        icon       = "https://assetdelivery.roblox.com/v1/asset/?id=" ..
                     (fishData.Data.Icon:match("%d+") or tostring(fishData.Data.Icon)),
    }
