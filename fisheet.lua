-- fisheet.lua — FishShit Notifier (Fluent UI) • embed styled like example #2
...
-- rbxassetid://123 → URL image Roblox (Discord bisa ambil)
...
-- ====== EMBED BUILDER (format seperti contoh #2) ======
...
local function build_embed(args)
  ...
  return {
    ...
    fields = {
      ...
      -- (Sesuai permintaan: TANPA Total Caught & Bag Size)
    },
...
do
  ...
  Tabs.Status:AddButton({
    ...
    Description = "Send styled embed like example #2",
    Callback = function()
      StatusLabel:SetDesc("Testing webhook with Robot Kraken…")

      -- Robot Kraken dummy (thumbnail akan muncul via converter)
      ...
      -- ambil icon dari list.lua kalau tersedia; fallback ke id ini
